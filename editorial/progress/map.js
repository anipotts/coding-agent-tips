...
const link = (text, href) => { const a = el('a', text); a.href = safeUrl(href) ?? '#'; a.target = '_blank'; a.rel = 'noopener'; return a; };
...
function matchesSection(section, page) {
  ...
  const relevant = !q || [page.title, page.scope, section.title, section.text].join(' ').toLowerCase().includes(q);
  const c = section.counts;
  const filters = {
    all: true, review: section.hasContent && section.review !== 'Ani reviewed', accepted: section.review === 'Ani reviewed' || section.observations.some(o => o.matches && ['preserve','adopted-heading'].includes(o.status)) || section.voice === 'preserve original',
...
function renderSummary() {
  ...
  $('#next').replaceChildren(...data.next.map(step => { const a = el('a', step.title); a.href = '#detail'; a.append(el('small', step.detail)); a.addEventListener('click', e => { e.preventDefault(); selected=step.file; $('#scope').value='all'; $('#filter').value='all'; $('#query').value=''; openIds.add(`${step.file}#${step.anchor}`); render(); document.getElementById(`${step.file}#${step.anchor}`)?.scrollIntoView({block:'start'}); }); return a; }));
}
...
function renderDetail(page) {
  ...
  for(const q of page.questions)node.append(el('p','Needs your input: '+(typeof q==='string'?q:JSON.stringify(q)),'page-note'));
